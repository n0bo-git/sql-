...
public interface GoodsMapper extends BaseMapper<Goods> {
     ...
     */
    @Select("<script>" +
        "SELECT g.goods_code, g.manu_code, g.goods_desc, g.unit_price, " +
        "m.manu_name " +
        "FROM goods g " +
     ...
     * 查询单条商品详情（含厂家名称）
     */
    @Select("SELECT g.goods_code, g.manu_code, g.goods_desc, g.unit_price, " +
        "m.manu_name " +
        "FROM goods g " +
        "LEFT JOIN manufacturer m ON g.manu_code = m.manu_code " +
     ...
     * 查询商品封面图
     */
    @Select("SELECT image_url FROM goods_image WHERE goods_code = #{goodsCode} AND is_cover = 1 ORDER BY sort_order LIMIT 1")
