...
-- 在 Navicat 中打开此文件，选中 demo 库，点击"运行"
-- ============================================================
...
INSERT IGNORE INTO `goods_category` VALUES
...
INSERT IGNORE INTO `goods_image` VALUES
...
-- ============================================================
-- 4. goods_supply 商品供应关系
-- ============================================================
CREATE TABLE IF NOT EXISTS `goods_supply` (
  `goods_code` varchar(20) NOT NULL COMMENT '商品编号',
  `supplier_code` varchar(20) NOT NULL COMMENT '供应商编号',
  ...
  KEY `fk_gs_supplier` (`supplier_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商品供应关系表';

INSERT IGNORE INTO `goods_supply` VALUES
...
-- ============================================================
-- 6. merchant_review 商家评价
-- ============================================================
CREATE TABLE IF NOT EXISTS `merchant_review` (
  `review_id` varchar(20) NOT NULL COMMENT '评价编号',
  `cust_code` varchar(20) NOT NULL COMMENT '顾客编号',
  `target_type` varchar(20) NOT NULL COMMENT '评价对象类型 manufacturer/supplier',
  `target_code` varchar(20) NOT NULL COMMENT '评价对象编号',
  ...
  `review_status` tinyint(1) DEFAULT 1 COMMENT '审核状态 1=通过 0=待审 -1=驳回',
  `review_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '评价时间',
  PRIMARY KEY (`review_id`),
  KEY `idx_target` (`target_type`, `target_code`),
  KEY `idx_cust` (`cust_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商家评价表';

INSERT IGNORE INTO `merchant_review` VALUES
('MR001', 'C001', 'manufacturer', 'M001', 5, '华为的产品质量一直很好，售后也很到位！', '感谢您的支持与信赖，我们会继续努力！', '2024-01-11 10:00:00', 0, 1, '2024-01-10 16:00:00'),
('MR002', 'C002', 'manufacturer', 'M003', 4, '格力空调用了这么多年，质量可靠。不过价格稍微贵了点。', '谢谢您的建议，我们将持续优化产品性价比。', '2024-01-12 14:00:00', 0, 1, '2024-01-11 09:30:00'),
('MR003', 'C003', 'manufacturer', 'M006', 3, '联想笔记本还行，但这次买的有轻微亮点，换过一次了。', '非常抱歉给您带来不便，我们已安排专人跟进处理。', '2024-01-13 16:00:00', 0, 1, '2024-01-12 11:00:00'),
('MR004', 'C003', 'manufacturer', 'M007', 5, '大疆无人机确实牛，技术全球领先！', NULL, NULL, 0, 1, '2024-01-12 11:10:00'),
('MR005', 'C005', 'manufacturer', 'M003', 4, '格力的产品总体满意，安装服务可以再快一点。', '感谢您的反馈，我们已优化安装调度系统。', '2024-01-15 09:00:00', 0, 1, '2024-01-14 15:00:00'),
('MR006', 'C005', 'supplier', 'S003', 5, '广州鑫源的包装材料质量很好，包装严实，物流快！', '谢谢支持，期待与您继续合作！', '2024-01-15 10:00:00', 0, 1, '2024-01-14 15:30:00'),
('MR007', 'C006', 'manufacturer', 'M001', 5, '华为笔记本办公用非常流畅，颜值也高。', '非常感谢您的好评！', '2024-01-16 11:00:00', 0, 1, '2024-01-15 10:00:00'),
('MR008', 'C007', 'manufacturer', 'M002', 4, '小米性价比没得说，希望品控能再加强。', '感谢您的建议，我们一直在努力提升品质。', '2024-01-17 14:00:00', 0, 1, '2024-01-15 12:00:00'),
('MR009', 'C008', 'manufacturer', 'M004', 4, '海尔家电用了很多年，售后服务特别好！', '服务好是我们的宗旨，谢谢您的认可！', '2024-01-17 15:00:00', 0, 1, '2024-01-16 17:00:00'),
('MR010', 'C004', 'manufacturer', 'M007', 5, '太喜欢大疆了，每次带出去飞行都引来围观！', NULL, NULL, 1, 1, '2024-01-20 10:00:00'),
('MR011', 'C001', 'supplier', 'S001', 5, '华强供应链发货非常及时，合作很顺畅。', '感谢您的认可，我们将一如既往提供优质服务。', '2024-01-21 09:00:00', 0, 1, '2024-01-20 14:00:00'),
('MR012', 'C006', 'supplier', 'S004', 3, '中科创新的芯片质量可以，但交货周期偏长了一些。', '感谢您的反馈，我们正在扩大产能缩短交期。', '2024-01-22 10:00:00', 0, 1, '2024-01-21 11:00:00');
...
CREATE TABLE IF NOT EXISTS `order_main` (
  ...
  PRIMARY KEY (`order_id`, `cust_code`),
  KEY `idx_cust` (`cust_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='订单主表';

INSERT IGNORE INTO `order_main` VALUES
('ORD20240101001', '2024-01-01 10:30:00', 'C001', '送货上门', 1, '2024-01-03', 2.50, 15.00, '已付'),
('ORD20240102001', '2024-01-02 14:20:00', 'C002', '快递', 1, '2024-01-04', 1.80, 10.00, '已付'),
('ORD20240103001', '2024-01-03 09:15:00', 'C003', '工作日配送', 1, '2024-01-05', 5.00, 25.00, '已付'),
('ORD20240104001', '2024-01-04 16:45:00', 'C001', '送货上门', 1, '2024-01-06', 3.20, 18.00, '已付'),
('ORD20240105001', '2024-01-05 11:00:00', 'C005', '周末配送', 1, '2024-01-07', 8.00, 35.00, '已付'),
('ORD20240106001', '2024-01-06 20:30:00', 'C004', '送货上门', 0, NULL, 0.00, 0.00, '未付'),
('ORD20240107001', '2024-01-07 08:00:00', 'C006', '快递', 1, '2024-01-09', 4.50, 20.00, '已付'),
('ORD20240108001', '2024-01-08 13:10:00', 'C007', '送货上门 轻拿轻放', 1, '2024-01-10', 2.00, 12.00, '已付'),
('ORD20240109001', '2024-01-09 17:00:00', 'C008', '快递', 1, '2024-01-11', 6.30, 30.00, '部分付'),
('ORD20240110001', '2024-01-10 10:00:00', 'C002', '送货上门', 1, '2024-01-12', 1.20, 8.00, '已付'),
('ORD20240111001', '2024-01-11 15:30:00', 'C003', '周末配送', 1, '2024-01-13', 3.80, 22.00, '已付'),
('ORD20240112001', '2024-01-12 12:00:00', 'C004', '送货上门', 1, '2024-01-14', 7.50, 32.00, '已付');
...
('PAY001', 'ORD20240101001', 'C001', 'alipay', 7797.00, 1, 'ALI20240101103000', '2024-01-01 10:31:00', '2026-06-05 14:58:42'),
('PAY002', 'ORD20240102001', 'C002', 'wechat', 3599.00, 1, 'WX20240102142100', '2024-01-02 14:21:00', '2026-06-05 14:58:42'),
('PAY003', 'ORD20240103001', 'C003', 'bank', 14287.00, 1, 'BANK20240103091600', '2024-01-03 09:16:00', '2026-06-05 14:58:42'),
('PAY004', 'ORD20240104001', 'C001', 'alipay', 4299.00, 1, 'ALI20240104164600', '2024-01-04 16:46:00', '2026-06-05 14:58:42'),
('PAY005', 'ORD20240105001', 'C005', 'balance', 7457.00, 1, NULL, '2024-01-05 11:01:00', '2026-06-05 14:58:42'),
('PAY006', 'ORD20240107001', 'C006', 'wechat', 8699.00, 1, 'WX20240107080100', '2024-01-07 08:01:00', '2026-06-05 14:58:42'),
('PAY007', 'ORD20240108001', 'C007', 'alipay', 5999.00, 1, 'ALI20240108131100', '2024-01-08 13:11:00', '2026-06-05 14:58:42'),
('PAY008', 'ORD20240109001', 'C008', 'card', 5947.00, 2, NULL, NULL, '2026-06-05 14:58:42'),
('PAY009', 'ORD20240109001', 'C008', 'alipay', 5947.00, 1, 'ALI20240109170200', '2024-01-09 17:02:00', '2026-06-05 14:58:42'),
('PAY010', 'ORD20240110001', 'C002', 'wechat', 259.00, 1, 'WX20240110100100', '2024-01-10 10:01:00', '2026-06-05 14:58:42'),
('PAY011', 'ORD20240111001', 'C003', 'bank', 6799.00, 1, 'BANK20240111153100', '2024-01-11 15:31:00', '2026-06-05 14:58:42'),
('PAY012', 'ORD20240112001', 'C004', 'alipay', 5935.00, 1, 'ALI20240112120100', '2024-01-12 12:01:00', '2026-06-05 14:58:42'),
('PAY013', 'ORD20240106001', 'C004', 'alipay', 1599.00, 0, NULL, NULL, '2026-06-05 14:58:42');

-- ============================================================
-- 10. shipping_address 收货地址
-- ============================================================
...
INSERT IGNORE INTO `shipping_address` VALUES
('ADDR001', 'C001', '张三', '13800138001', '北京市', '北京市', '朝阳区', '建国路88号SOHO现代城A座1205', '100020', 1, '2026-06-05 14:58:42'),
('ADDR002', 'C001', '张太太', '13900139099', '北京市', '北京市', '海淀区', '中关村东路1号清华科技园B座808', '100084', 0, '2026-06-05 14:58:42'),
('ADDR003', 'C002', '李四', '13900139002', '上海市', '上海市', '浦东新区', '张江路168号盛大网络园区3号楼', '200120', 1, '2026-06-05 14:58:42'),
('ADDR004', 'C003', '王五', '13700137003', '广东省', '广州市', '天河区', '体育西路55号天河城广场', '510620', 1, '2026-06-05 14:58:42'),
('ADDR005', 'C004', '赵六', '13600136004', '广东省', '深圳市', '南山区', '科技园路12号腾讯滨海大厦', '518057', 1, '2026-06-05 14:58:42'),
('ADDR006', 'C005', '陈小红', '13500135005', '浙江省', '杭州市', '西湖区', '文三路200号支付宝大楼', '310012', 1, '2026-06-05 14:58:42'),
('ADDR007', 'C006', '刘强', '13400134006', '四川省', '成都市', '武侯区', '人民南路33号天府软件园', '610041', 1, '2026-06-05 14:58:42'),
('ADDR008', 'C007', '周美玲', '13300133007', '江苏省', '南京市', '鼓楼区', '中山北路18号紫峰大厦', '210008', 1, '2026-06-05 14:58:42'),
('ADDR009', 'C008', '吴大伟', '13200132008', '湖北省', '武汉市', '洪山区', '珞喻路77号光谷广场', '430070', 1, '2026-06-05 14:58:42');
...
(1, 'admin', '123', '管理员', '13677889988', 'admin@xxx.com', '江苏南京', 'http://localhost:9999/file/download/汤姆.jpg', '男', 27, '我是管理员', 'ADMIN'),
(2, 'tom', '123', '汤姆', '13988776699', 'jerry@qq.com', '北京', 'http://localhost:9999/file/download/汤姆.jpg', '男', 24, '我爱你，亲爱的姑娘', 'USER'),
(3, 'jerry', '123', '杰瑞', '15098765321', 'tom@qq.com', '上海', 'http://localhost:9999/file/download/杰瑞.jpg', '男', 25, '我爱你，亲爱的姑娘', 'USER');
